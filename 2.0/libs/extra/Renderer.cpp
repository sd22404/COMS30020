...
void Renderer::drawLine(CanvasPoint &p0, CanvasPoint &p1, uint32_t &colour) {
	...
	if (p0.isOffScreen(window.width, window.height) && p1.isOffScreen(window.width, window.height)) return;
...
void Renderer::fillTriangle(CanvasTriangle &triangle, uint32_t &colour) {
	if (triangle.v0().isOffScreen(window.width, window.height) &&
		triangle.v1().isOffScreen(window.width, window.height) &&
		triangle.v2().isOffScreen(window.width, window.height)) return;
